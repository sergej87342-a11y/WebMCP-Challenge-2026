...
- `create_booking` отсутствует и не реализован.
...
- `check_availability` реализован и реально проверен в Chrome; `create_booking` не реализован.
...
Перед началом `create_booking` отдельно подтвердить объём, синтетическое хранилище и атомарную защиту слота. До такого решения `create_booking` не реализовывать.
